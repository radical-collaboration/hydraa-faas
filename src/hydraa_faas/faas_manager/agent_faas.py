...
class AgentFaas:
    ...
    async def _process_bulk(self, bulk):
        ...
        for operation, task in bulk:
            if operation == 'deploy':
                ...
                tasks.append(
                    self._invoke_function(task.name, task.payload or {}))
    ...
    async def _invoke_function(self, func_name: str, payload: Any) -> Any:
        ...
        try:
            ...
            self.outgoing_q.put(
                f'Function {func_name} invocation failed: {str(e)}')
            raise
    ...
    async def _list_functions(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.info(
                f"Listed {len(filtered_functions)} agent functions")
                ...
                for task_name, task_info in self._deployed_functions.items():
                    ...
                    if hasattr(task, 'done') and task.done():
                        if hasattr(task, 'exception') and task.exception():
                            failed += 1
                            self.logger.error(
                                f"Task {task_name} failed: {task.exception()}")
                        else:
                            done += 1
    ...
    async def _cleanup_functions(self):
        ...
        if self.http_client is None:
            self.logger.warning(
                "HTTP client not available for cleanup, creating temporary one.")
            async with httpx.AsyncClient(timeout=30.0) as client:
                cleanup_tasks = [
                    client.delete(f"{self.agent_url}/function/{func_name}") for
                    func_name in
                    self._deployed_functions]
        ...
        self.logger.info(
            f"Cleaning up {len(self._deployed_functions)} functions using shared client...")
        cleanup_tasks = []
